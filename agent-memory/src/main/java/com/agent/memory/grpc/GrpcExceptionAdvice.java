...
/**
 * gRPC 异常翻译器（Plan 03 T10，复用 agent-repo / agent-model-gateway 模式）。
 *
 * <p>将 {@link BusinessException} 翻译为 gRPC {@link Status} 并通过
 * {@code observer.onError} 下发，避免服务方法抛未捕获异常导致 channel 异常断开。</p>
 *
 * <p>错误码 → gRPC Status 映射（对齐 Plan 03 T10 设计）：</p>
 * <ul>
 *   <li>MEMORY_NOT_FOUND(404) → NOT_FOUND</li>
 *   <li>EMBEDDING_FAILED(500) → INTERNAL（向量化失败属内部错误）</li>
 *   <li>VECTOR_STORE_ERROR(500) → INTERNAL</li>
 *   <li>PARAM_INVALID(400) / VALIDATION_FAILED(400) → INVALID_ARGUMENT</li>
 *   <li>RATE_LIMITED(429) / QUOTA_EXCEEDED(429) → RESOURCE_EXHAUSTED</li>
 *   <li>DEPENDENCY_DOWN(503) / CIRCUIT_OPEN(503) → UNAVAILABLE</li>
 *   <li>TIMEOUT(504) / MODEL_TIMEOUT(504) → DEADLINE_EXCEEDED</li>
