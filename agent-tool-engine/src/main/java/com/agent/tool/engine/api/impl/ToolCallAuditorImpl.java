...
/**
 * F8 工具调用审计实现 (tool_call_log 内存持久化)。
 *
 * <p>骨架阶段使用同步 List 记录审计日志, audit() 自动补全 logId。
 * 生产实现应替换为 ClickHouse / Elasticsearch 异步落库。</p>
 */
@Component
public class ToolCallAuditorImpl implements ToolCallAuditor {
    ...
    /** 内存审计日志 (同步访问, 保证线程安全)。 */
    private final List<ToolCallAuditLog> logs = new ArrayList<>();
    ...
    public void audit(ToolCallAuditLog logEntry) {
        ...
        if (logEntry.getOccurredAt() == null) {
            logEntry.setOccurredAt(java.time.Instant.now());
        }
        synchronized (logs) {
            logs.add(logEntry);
        }
        log.debug("审计日志写入: logId={}, traceId={}, toolId={}, status={}",
                logEntry.getLogId(), logEntry.getTraceId(), logEntry.getToolId(), logEntry.getStatus());
    }
