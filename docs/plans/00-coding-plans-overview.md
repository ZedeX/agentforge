...
### 当前总体进度（截至 Wave 34，CI streak=33）
...
| 03 | [agent-memory](./03-agent-memory-plan.md) | agent-memory(8088/9088) | 7/10 | 🔄 进行中 | Wave 30~34 |
...
#### Plan 03 — agent-memory（🔄 进行中，7/10 Task）
| Task | 状态 | 完成 Wave | 说明 |
|---|---|---|---|
...
| T5 EmbeddingClient | ⏳ | — | 骨架已有（mock 实现），待接入 HTTP /v1/embeddings |
...
### 阶段 B：核心引擎层（P1）—— 🔄 进行中
- **Plan 03（memory）** 🔄 7/10，T5/T6/T10 待做
...
| 优先级 | Plan / Task | 理由 |
|---|---|---|
| 高 | Plan 03 T5 EmbeddingClient | 解锁 T6/T7 完整 novelty 计算；自包含（MockWebServer 即可测试） |
